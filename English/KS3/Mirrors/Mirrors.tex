...
\begin{shaded}
\textbf{Exercise \theExercise \stepcounter{Exercise}} : Measure the angle of incidence in figure~\ref{fig:lawofreflection}.
\end{shaded}
...
\begin{center}
\begin{tikzpicture}
\draw (0,0) arc [radius=8.0, start angle=0, end angle= 40] node[left] {First wavefront};
\draw (1,0) arc [radius=9.0, start angle=0, end angle= 40] node[above right] {Next wavefront};
\foreach \i in {1,...,5}
{
  \draw ({(7.9*cos(5*\i+5)-8)},{7.9*sin(5*\i+5)})--({(8.1*cos(5*\i+5)-8)},{8.1*sin(5*\i+5)});
  \draw[dashed] ({8*(cos(5*\i+5)-1)},{8*sin(5*\i+5)}) circle[radius=1];
}
\node at (-4,1) {Circle with radius = wavelength };
\end{tikzpicture}
...
\begin{figure}\begin{center}
\includegraphics[scale=1.2]{plane_wave}
